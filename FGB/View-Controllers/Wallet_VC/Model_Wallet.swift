...
class Model_Wallet {
    static let shared = Model_Wallet()
    ...
    var arr_wallet_use = [Model_Wallet]()
    
    func func_wallet_use(completionHandler:@escaping (String)->()) {
        let param = ["customer_id":Model_Walk_Through.shared.customer_id]
        APIFunc.postAPI(url: k_base_url+"wallet_use", parameters: param) {
            (dict_JSON) in
            print(dict_JSON)
            
            self.arr_wallet_use.removeAll()
            if dict_JSON["status"] as? String == "success" {
                let arr_result = dict_JSON["result"] as! [[String:Any]]
                for dict in arr_result {
                    self.arr_wallet_use.append(self.func_set_data(dict: dict))
                }
    ...
    private func func_set_data(dict:[String:Any]) -> Model_Wallet {
        let model = Model_Wallet()
